...
function midiNoteOut(ch, noteNum, vel, start_time, end_time){
    console.log("MIDI Out: ", ch, noteNum, vel, start_time, end_time);
    activeMidiOutputDevice.send([MIDI_EVENT_ON+ch, noteNum, vel],
				window.performance.now() + start_time + MIDI_LATENCY_MS);
    activeMidiOutputDevice.send([MIDI_EVENT_OFF+ch, noteNum, vel],
...
async function playNoteListMIDI(notes){ // expects quantized NoteList.notes array
	...
	ch = n.instrument;
	midiNoteOut(ch, n.pitch, 100, start_time, end_time);
    }
}
